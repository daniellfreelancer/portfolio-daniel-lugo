import React from "react";
import "../styles/Training.css";

export default function Training() {
  return (
    <>
      <div>
        <hr className="divider" />
      </div>

      <div className="div-separetor">
        <h2>Capacitación</h2>
      </div>
      <div className="div-container-education">
        <div className="container flex-wrap">
          <div className="card-edu">
            <div className="face face1">
              <div className="content">
                <img
                  src="https://github.com/Jhonierpc/WebDevelopment/blob/master/CSS%20Card%20Hover%20Effects/img/code_128.png?raw=true"
                  alt="logo"
                />
                <h3>FrontEnd Developer React</h3>
              </div>
            </div>
            <div className="face face2">
              <div className="content">
                <h3> (Ago-2021 / May 2022)</h3>
                <p>
                  Carrera de desarrollador front-end con enfoque a React.js para
                  desarrollo de Single Page Aplication, utilizando HTML, CSS,
                  SASS, Jquery, Javascript, Bootstrap, Firebase, Git & GitHub,
                  Fetch .
                </p>
              </div>
            </div>
          </div>
          <div className="card-edu">
            <div className="face face1">
              <div className="content p-5 ">
                <img
                  src="https://github.com/Jhonierpc/WebDevelopment/blob/master/CSS%20Card%20Hover%20Effects/img/launch_128.png?raw=true"
                  alt="logomin"
                />
                <h3>FullStack Developer MERN & Mobile</h3>
              </div>
            </div>
            <div className="face face2">
              <div className="content">
                <h3>(Jul-2022 / Oct 2022</h3>
                <p>
                  Bootcamp con +700 Horas full-time utilizando metodologías
                  ágiles (SCRUM), simulando un ambiente de trabajo real
                  utilizando tecnologías como MongoDB, Express, React.js,
                  Node.js, React Native, Heroku, Git & GitHub, GitHub Project,
                  Axios, Redux.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
